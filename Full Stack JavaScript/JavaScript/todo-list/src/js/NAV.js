export default class NAV {
	constructor() {
		...
		this.menuPrio = document
			.getElementById('menu-priority')
			.querySelectorAll('li')
		this.menuProject = document
	...
	initEventListeners() {
		window
			.matchMedia('(max-width: 1290px')
			.addEventListener('change', () => this.toggleSidebar())
	...
	filterTasks() {
		...
		const selectedDateOption = Array.from(this.menuTask).find((li) =>
			li.classList.contains('active')
		).innerText
		const selectedPriorityOption = Array.from(this.menuPrio).find((li) =>
			li.classList.contains('active')
		).innerText
		const selectedProjectOption = Array.from(this.menuProject).find((li) =>
			li.classList.contains('active')
		).innerText
		...
		taskElements.forEach((taskElement) => {
			...
			const shouldShowTask =
				(selectedDateOption === 'All Tasks' ||
					(selectedDateOption === 'Daily' &&
						this.isToday(taskDate, userSelectedDate)) ||
					(selectedDateOption === 'Weekly' &&
						this.isThisWeek(taskDate, userSelectedDate))) &&
				(selectedPriorityOption === 'All Priorities' ||
					taskPriority.includes(selectedPriorityOption.toLowerCase())) &&
				(selectedProjectOption === 'All Projects' ||
					taskProject === selectedProjectOption)
